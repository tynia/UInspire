...
bool CUIWndManager::InitWndInstance( IUIWnd* initWnd, HWND& hInitWnd, const HWND& hPareant, DWORD style )
{
   ...
   {
      LogError( "RegisterWndClass %s fail", INSPIRE_INTERNAL::CharConverter( initWnd->GetID() ).GetUTF8() );
      return false;
   }

   INSPIRE_CLIENT::CRect rect = initWnd->GetRect();
   int cx = 0;
   int cy = 0;
   if ( hPareant )
   {
      INSPIRE_CLIENT::IUIWnd* wnd = GetUIBase( hPareant )->GetMainWnd();
...
LRESULT CALLBACK CUIWndManager::__WindowProcess( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
   ...
   case WM_PAINT :
      {
         INSPIRE_CLIENT::IUIWnd* wnd = static_Instance->GetUIBase( hWnd )->GetMainWnd();
         LIB_ASSERT( wnd );
...
void CUIWndManager::Draw( const HWND& hWnd, INSPIRE_CLIENT::IUIWnd* wnd )
{
